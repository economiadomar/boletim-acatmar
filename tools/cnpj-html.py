...
put('nota', f'<li data-en="Companies: Federal Revenue CNPJ open data ({REF_EN}), establishments with active status and main CNAE in one of the ten nautical subclasses; includes MEI and companies without employees, which is why it exceeds RAIS. Marinas and boat garages have no CNAE of their own and can only be counted by the Port Authority and city halls.">Empresas: dados abertos do CNPJ da Receita Federal ({REF}), estabelecimentos com situação ativa e CNAE principal em uma das dez subclasses náuticas; inclui MEI e empresas sem empregados, por isso é maior que a RAIS. Marinas e garagens náuticas não têm CNAE próprio e só podem ser contadas pela Capitania dos Portos e pelas prefeituras.</li>')
