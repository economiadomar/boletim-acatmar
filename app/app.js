...
function home(){
  ...
  var spons=D.patrocinadores.map(function(c,i){var modo={'Patrocínio institucional':'grande','Patrocínio':'grande','Realização':'medio','Apoio institucional':'duplo'}[c.cota]||'lado';return '<div class="strip-cota strip-'+modo+'"><span>'+esc(c.cota)+'</span><div>'+c.empresas.map(function(p){var sc=Math.round((p.escala||1)*100);return '<a href="#/patrocinadores">'+(p.logo?'<img src="'+p.logo+'" alt="'+esc(p.nome)+'" style="max-height:'+sc+'%;max-width:'+sc+'%">':'<span class="logo-txt logo-txt-sm">'+esc(p.nome)+'</span>')+'</a>';}).join('')+'</div></div>';}).join('');
